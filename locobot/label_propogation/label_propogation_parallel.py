...
def propogate_label(
    ...
    # steps are copeid from pyrobot (https://github.com/facebookresearch/pyrobot/blob/13a70d2427417756504ca4ff3cf99a33f15abbde/src/pyrobot/utils/util.py#L233-L241)
    ...
    for img_indx in range(image_range[0], image_range[1]):
        ...
        for i, (req_pts_in_world, pix_color) in enumerate(
            ...
            # pts_in_cur_img need not be integers as they are generated by division (pts_in_cur_img /= pts_in_cur_img[:, 2].reshape([-1, 1]))
            # however img index should always be an integer
            # to convert float to integer, we try to map the float to all possible nearest integer valid index
